...
namespace catalogue {
	...
	void TransportCatalogue::AddRoute(const std::string_view& name,
		...
		buses_id_.insert({ route.name, buses_id_.size() });
	...
	void TransportCatalogue::AddStop(const std::string_view name,
		...
		stops_id_.insert({ stop.name, stops_id_.size() });
		buses_in_stop_[&stop];
	}
	...
	void TransportCatalogue::SetDistances() {
		for (const auto& stop : stops_name_) {
			for (auto& [to, distance] : stop.second->stops_distance) {
				SetDistanceBetweenStops(stop.second, stops_name_.at(to), distance);
			}
		}
	}
	...
	size_t TransportCatalogue::GetStopId(std::string_view name) const {
		return stops_id_.at(name);
	}

	size_t TransportCatalogue::GetBusId(std::string_view name) const {
		return buses_id_.at(name);
	}
	...
	const std::deque<Bus>* TransportCatalogue::GetBusesDeque() const {
		return &buses_;
	}
}